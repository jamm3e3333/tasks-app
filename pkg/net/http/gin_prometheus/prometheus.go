...
const (
	// LabelURL is name of the label containing the route url
	LabelURL = "url"

	// LabelMethod is name of the label containing the method used
	// in the request. By default, if method isn't one of the known
	// methods "other", will be used instead.
	LabelMethod = "method"

	// LabelStatusCode is name of the label containing response status code
...
var defaultRequestDurationBuckets = []float64{0.005, 0.01, 0.025, 0.05, 0.1, 0.25, 0.5}
...
type Config struct {
	...
	// Labels allows setting additional labels on HTTP metrics
	// If label with same name appears more than once, it's definition
	// is replaced. This allows overriding even default labels like LabelURL,
	// LabelMethod and LabelCode.
	Labels []Label
...
// Measure is same as MeasureWith, but uses the default Prometheus registerer
func Measure(config Config) gin.HandlerFunc {
	return MeasureWith(prometheus.DefaultRegisterer, config)
}

// MeasureWith registers metrics with the Prometheus registerer and returns
// Gin middleware for recording request metrics. If metrics can't be registered
// it will panic.
...
// record does the heavy-lifting of updating the metrics
func (m *metricsHandler) record(c *gin.Context) {
	...
	if !ok {
		// try recording size of response body
		// this only works if the handler actually reads
		// it otherwise it will get discraded somewhere
		// in http package
	...
	requestDurationSeconds := float64(time.Since(requestStart).Seconds())
...
func estimateRequestSize(r *http.Request, bodySize int64) int64 {
	...
	// we assume Proto is mentioned in the request
	// but this is not the case in HTTP/2
	s += len(r.Proto)

	// size of header names and values
	// ignores header encoding i.e.
	// HTTP2 headers are represented in binary and compressed
	// HTTP1 headers are represented textually and concatenated with special characters
